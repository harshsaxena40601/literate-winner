  ...
  return (
    <section className="col-span-12">
      <div className="grid grid-cols-12 gap-3 md:gap-4">
        <div className="col-span-12 lg:col-span-4 rounded-2xl border bg-soft p-4">
          ...
          <div className="mt-4">
            <button onClick={() => { localStorage.clear(); location.reload(); }} className="text-sm px-3 py-2 rounded-xl border bg-soft hover:bg-bg-muted">
              Reset demo data
            </button>
          </div>
        </div>

        <div className="col-span-12 lg:col-span-8 rounded-2xl border bg-soft p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-1 inline-flex gap-2">
                <button onClick={()=>setTheme('light')} className={`px-3 py-1 rounded-lg border ${theme==='light' ? 'bg-[color:var(--tw-color-accent)] text-white border-orange-500' : ''}`}>Light</button>
                <button onClick={()=>setTheme('dark')} className={`px-3 py-1 rounded-lg border ${theme==='dark' ? 'bg-[color:var(--tw-color-accent)] text-white border-orange-500' : ''}`}>Dark</button>
              </div>
            </div>
          </div>
