...
export default function AnalyticsPanel({ anaRange, setAnaRange, salesByDay, movingAvg, payModeAgg, topProducts, ww }) {
  return (
    <section className="col-span-12">
      <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-soft p-3 md:p-4 mb-3 flex items-center justify-between gap-2">
        ...
        <div className="inline-flex items-center gap-2 text-sm"><span>Range</span>
          {[7,14,30,90].map(n => (
            <button key={n} onClick={()=>setAnaRange(n)} className={`px-2 py-1 rounded-lg border ${anaRange===n? 'bg-[color:var(--tw-color-accent)] text-white border-orange-500' : ''}`}>{n}D</button>
          ))}
        </div>
      </div>

      <div className="grid grid-cols-12 gap-3 md:gap-4">
        <div className="col-span-12 lg:col-span-8 rounded-2xl border bg-soft p-3 md:p-4">
        ...
        <div className="col-span-12 lg:col-span-4 rounded-2xl border bg-soft p-3 md:p-4">
        ...
        <div className="col-span-12 rounded-2xl border bg-soft p-3 md:p-4">
