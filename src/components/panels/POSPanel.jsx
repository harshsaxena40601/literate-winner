        ...
        <div className="card backdrop-blur p-4 sticky top-24 shadow-sm max-h-[70vh] overflow-auto">
          ...
          {cart.length===0 ? (
            ...
            <div className="space-y-2">
              {cart.map((line)=>{
                ...
                return (
                  <div key={line.id} className={`flex items-center gap-2 rounded-xl border p-3 bg-soft ${focused? 'ring-2 ring-orange-300' : 'border-neutral-800'}`} onMouseEnter={()=>{focusedCartIdRef.current=line.id;}}>
          ...
          <div className="mt-4 border-t dark:border-neutral-800 pt-4 space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={()=>printReceipt()} disabled={cart.length===0} className={`flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-neutral-700 dark:text-neutral-200 border ${cart.length===0? 'bg-neutral-100 cursor-not-allowed' : 'bg-soft hover:bg-bg-muted'}`}><Printer className="h-5 w-5"/> Preview</button>
