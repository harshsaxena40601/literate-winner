...
export default function ProductCard({ product, rem, low, oos, onAdd }) {
  return (
    <button
      ...
      disabled={oos}
      className={`group relative rounded-2xl overflow-hidden border transition-all ${oos? 'bg-neutral-100 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 opacity-70 cursor-not-allowed' : 'bg-card hover:shadow-md transition-transform duration-150 hover:-translate-y-0.5 border-neutral-200 dark:border-neutral-700 hover:border-brand-200'}`}
    >
      ...
      <div className="p-3 text-left">
        ...
        <div className="mt-2 flex items-center justify-between">
          <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full font-medium ${oos? 'bg-[color:var(--tw-color-accent)]/20 text-[color:var(--tw-color-accent)]' : low? 'bg-yellow-500/20 text-yellow-400' : 'bg-emerald-500/20 text-emerald-400'}`}><Package className="h-3 w-3"/>{oos? 'Out of stock' : `${rem} left`}</span>
